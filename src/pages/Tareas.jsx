...
const Tareas = () => {
  ...
  const obtenerTareas = async () => {
    ...
    try {
      ...
      const categorias = [
        ...new Set(tareasConSubtareas.map((t) => t.categoria).filter(Boolean)),
      ];
  ...
  const onDragEndSubtareas = (result, tareaId) => {
    ...
    setTareas((prev) =>
      prev.map((t) => (t._id === tareaId ? updatedTarea : t))
    );
  ...
  const filtrarTareas = () => {
    ...
    if (estadoFiltro)
      filtradas = filtradas.filter((t) => t.estado === estadoFiltro);
    if (asignadoAFiltro)
      filtradas = filtradas.filter((t) =>
        t.asignadoA?.toLowerCase().includes(asignadoAFiltro.toLowerCase())
      );
    if (fechaInicio && !fechaFin)
      filtradas = filtradas.filter((t) => t.fecha === fechaInicio);
    if (fechaInicio && fechaFin)
      filtradas = filtradas.filter(
        (t) =>
          new Date(t.fecha) >= new Date(fechaInicio) &&
          new Date(t.fecha) <= new Date(fechaFin)
      );
        ...
        <div>
          ...
          <div className="sticky top-0 z-40 bg-gray-900 p-4 rounded-t-lg border-b border-gray-700 mb-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4">
              ...
              <select
                value={asignadoAFiltro}
                onChange={(e) => setAsignadoAFiltro(e.target.value)}
                className="bg-gray-800 text-white p-2 rounded text-sm"
              >
                <option value="">Todos</option>
                ...
                <input
                  ...
                  onKeyPress={(e) => {
                    if (e.key === 'Enter' && nuevaSubtarea.trim()) {
                      setSubtareas([
                        ...subtareas,
                        { texto: nuevaSubtarea.trim(), completada: false },
                      ]);
                      setNuevaSubtarea('');
                    }
                  }}
                                  ...
                                  <input
                                    ...
                                    onChange={() => {
                                      ...
                                      if (!s.completada) {
                                        mostrarNotificacion(
                                          '✅ Subtarea completada',
                                          `"${s.texto}" ha sido completada.`
                                        );
                                      }
                                    }}
                                    className="text-green-500"
                                  />
                                  {editandoSubtarea?.index === i &&
                                  editandoSubtarea?.origen === 'form' ? (
                                    <input
                                      type="text"
                                      ...
                                      onBlur={() => {
                                        ...
                                        updated[i] = {
                                          ...s,
                                          texto: editandoSubtarea.valor.trim() || s.texto,
                                        };
                                        setSubtareas(updated);
                                    ...
                                    <span
                                      className={`cursor-pointer ${
                                        s.completada ? 'line-through text-gray-400' : ''
                                      }`}
                                      onDoubleClick={() =>
                                        setEditandoSubtarea({
                                          origen: 'form',
          ...
          <div className="mt-6 p-4 bg-gray-800 rounded mb-6">
            ...
            {tareasRecientes.length > 0 ? (
                    ...
                    <button
                      ...
                      onClick={() => {
                        setTareaSeleccionada(
                          tareaSeleccionada?._id === t._id ? null : t
                        );
              ...
              <p className="text-gray-500 text-sm">
                No has visto ninguna tarea recientemente.
              </p>
            )}
          </div>
              ...
              >
                <h3 className="text-base sm:text-lg font-bold flex items-center gap-2 text-white">
                  {t.estado === 'completada' ? (
                    ...
                    <FaRegSquare className="text-yellow-300" />
                  )}{' '}
                  {t.titulo}
                </h3>
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                    ...
                    <button
                      onClick={() =>
                        setCompartirMenuAbierto(
                          compartirMenuAbierto === t._id ? null : t._id
                        )
                      }
                  ...
                  <button
                    onClick={async () => {
                      const token = localStorage.getItem('token');
                      const nuevoEstado =
                        t.estado === 'pendiente' ? 'completada' : 'pendiente';
                      ...
                      if (res.ok)
                        setTareas((prev) =>
                          prev.map((task) =>
                            task._id === t._id ? { ...task, estado: nuevoEstado } : task
                          )
                        );
                    }}
                      ...
                      <div className="mb-3">
                        ...
                        <ul className="list-disc list-inside space-y-1 mt-1">
                          {t.subtareas.map((sub, idx) => (
                            <li
                              key={idx}
                              className={sub.completada ? 'line-through text-gray-400' : ''}
                            >
                              {sub.texto}
                            </li>
                          ))}
